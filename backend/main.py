...
from update_face_data import update_photos_with_face_data
...
@app.post("/api/update-face-data")
async def update_face_data(background_tasks: BackgroundTasks):
    """Update existing photo metadata with face detection and recognition data."""
    global processing_status
    ...
    try:
        # Update status
        processing_status["is_processing"] = True
        processing_status["current_stage"] = "preparing_face_data_update"
        processing_status["processed_photos"] = 0
        processing_status["total_photos"] = 0
        processing_status["faces_detected"] = 0
        processing_status["error"] = None
        
        # Start background task
        background_tasks.add_task(update_face_data_task)
        
        return {"message": "Started updating photos with face detection and recognition data"}
    
    except Exception as e:
        processing_status["is_processing"] = False
        processing_status["error"] = str(e)
        raise HTTPException(status_code=500, detail=f"Error updating face data: {str(e)}")
...
async def update_face_data_task():
    """Background task to update photos with face detection and recognition data."""
    global processing_status
    ...
    try:
        ...
        # Update face data
        stats = await update_photos_with_face_data(DATA_DIR, update_status)
        
        # Mark as complete
        processing_status["is_processing"] = False
        processing_status["current_stage"] = "complete"
        processing_status["face_stats"] = stats
    
    except Exception as e:
        processing_status["is_processing"] = False
        processing_status["error"] = str(e)
        print(f"Error in background face data update: {str(e)}")
